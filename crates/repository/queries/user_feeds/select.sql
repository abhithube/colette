WITH unread_counts AS (
    ...
    INNER JOIN
        ...
        ON ufe.user_feed_id = uf.id AND NOT ufe.has_read
    GROUP BY uf.id
),
...
json_tags AS (
    SELECT
        uf.id AS uf_id,
        jsonb_agg(
            jsonb_build_object('id', t.id, 'title', t.title) ORDER BY t.title
...
WHERE
    ...
    AND (
        $4::bool
        OR CASE
            WHEN $5::uuid IS NULL THEN uf.folder_id IS NULL ELSE
                uf.folder_id = $5
        END
    )
    AND (
        ...
        OR EXISTS (
            SELECT t.*
            FROM jsonb_array_elements(jt.tags) t
            WHERE t ->> 'title' = any($7)
        )
    )
