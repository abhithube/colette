...
pub async fn insert(ex: impl PgExecutor<'_>, params: InsertParams<'_>) -> Result<Tag, sqlx::Error> {
    let row = sqlx::query_as!(
        Tag,
        "
   INSERT INTO tags (title, profile_id)
   VALUES ($1, $2)
RETURNING id,
          title,
          profile_id,
...
pub async fn update(ex: impl PgExecutor<'_>, params: UpdateParams<'_>) -> Result<Tag, sqlx::Error> {
    let row = sqlx::query_as!(
        Tag,
        "
   UPDATE tags
