...
impl PasswordStorage for AuthAdapter {
    ...
    async fn set_password_hash(
        ...
        match self {
            Self::Libsql(backend) => backend.set_password_hash(user_id, hash).await,
        }
    }
    ...
    async fn get_password_hash(
        ...
        match self {
            Self::Libsql(backend) => backend.get_password_hash(user_id).await,
        }
    }
}
...
impl SessionStorage for AuthAdapter {
    ...
    async fn create_session(&self, session: &Session) -> Result<Session, Self::Error> {
        match self {
            Self::Libsql(backend) => backend.create_session(session).await,
        }
    }

    async fn get_session(&self, token: &SessionToken) -> Result<Option<Session>, Self::Error> {
        match self {
            Self::Libsql(backend) => backend.get_session(token).await,
        }
    }

    async fn delete_session(&self, token: &SessionToken) -> Result<(), Self::Error> {
        match self {
            Self::Libsql(backend) => backend.delete_session(token).await,
        }
    }

    async fn cleanup_expired_sessions(&self) -> Result<(), Self::Error> {
        match self {
            Self::Libsql(backend) => backend.cleanup_expired_sessions().await,
        }
    }

    async fn delete_sessions_for_user(&self, user_id: &UserId) -> Result<(), Self::Error> {
        match self {
            Self::Libsql(backend) => backend.delete_sessions_for_user(user_id).await,
        }
    }
}
