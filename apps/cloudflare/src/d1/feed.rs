...
impl Creatable for D1FeedRepository {
    ...
    async fn create(&self, data: Self::Data) -> Self::Output {
        let feed_id = {
            let (sql, values) =
                colette_sql::feed::select_by_url(data.url.clone()).build_d1(SqliteQueryBuilder);
...
pub(crate) async fn create_feed_with_entries(
    ...
    let feed_id = {
        ...
        let (sql, values) =
            colette_sql::feed::insert(link, feed.title, url).build_d1(SqliteQueryBuilder);
    ...
    if !feed.entries.is_empty() {
        ...
        for chunk in insert_many.chunks(CHUNK_SIZE) {
            let (sql, values) =
                colette_sql::feed_entry::insert_many(chunk, feed_id).build_d1(SqliteQueryBuilder);
...
pub(crate) async fn link_entries_to_profiles(db: &D1Database, feed_id: i32) -> worker::Result<()> {
    let fe_ids = {
        let (sql, values) =
            colette_sql::feed_entry::select_many_by_feed_id(feed_id).build_d1(SqliteQueryBuilder);
...
pub(crate) async fn link_tags(
    ...
    let queries: Vec<(String, D1Values)> = vec![
        colette_sql::profile_feed_tag::delete_many_not_in_titles(&tags, profile_id)
            .build_d1(SqliteQueryBuilder),
        colette_sql::profile_feed_tag::insert_many(&insert_many, profile_id)
            .build_d1(SqliteQueryBuilder),
    ];
