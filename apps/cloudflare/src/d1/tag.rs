...
impl Creatable for D1TagRepository {
    ...
    async fn create(&self, data: Self::Data) -> Self::Output {
        ...
        let (sql, values) = colette_sql::tag::insert(Some(id), data.title.clone(), data.profile_id)
            .build_d1(SqliteQueryBuilder);
...
impl Updatable for D1TagRepository {
    ...
    async fn update(&self, params: Self::Params, data: Self::Data) -> Self::Output {
        if data.title.is_some() {
            let (sql, values) = colette_sql::tag::update(params.id, params.profile_id, data.title)
                .build_d1(SqliteQueryBuilder);
